...
int meth_main(int argc, char* argv[], int8_t mode) {
    ...
    while (status.num_reads >= core->opt.batch_size || status.num_bases>=core->opt.batch_size_bases) {
      ...
      //  fprintf(stderr,"batch idx %d\n", batch_idx);
         
        //load a databatch
        status = load_db(core, db);
        db->batch_idx = batch_id;
    ...
    fprintf(stderr, "\n[%s] FPGA alignment time: %.3f sec", __func__, core->align_FPGA_time);
    fprintf(stderr, "\n[%s]     - call kernel_0 number  : %d ", __func__, core->call_kernel_0_num);
    fprintf(stderr, "\n[%s]     - call kernel_1 number  : %d ", __func__, core->call_kernel_1_num);
    fprintf(stderr, "\n[%s]     - new bo time  : %.3f sec", __func__, core->new_bo_time);
    fprintf(stderr, "\n[%s]     - fill bucket time: %.3f sec", __func__, core->fill_bucket_time);
    fprintf(stderr, "\n[%s]     - sync to device: %.3f sec", __func__, core->sync_to_device);
    fprintf(stderr, "\n[%s]     - kernel run time: %.3f sec", __func__, core->kernel_run);
    fprintf(stderr, "\n[%s]     - sync from device: %.3f sec", __func__, core->sync_from_device);

    fprintf(stderr, "\n[%s]        ultra-long reads", __func__);
    fprintf(stderr, "\n[%s]         -  ultra-long reads num: %d", __func__, core->ultra_long_reads_num);
    fprintf(stderr, "\n[%s]         -  call ultra-kernel number: %d ", __func__, core->call_kernel_2_num);
    fprintf(stderr, "\n[%s]         -  fill bucket time: %.3f sec", __func__, core->fill_bucket_time_ultra);
    fprintf(stderr, "\n[%s]         -  sync to device time: %.3f sec", __func__, core->sync_to_device_ultra);
    fprintf(stderr, "\n[%s]         -  kernel run time: %.3f sec", __func__, core->kernel_run_ultra);
    fprintf(stderr, "\n[%s]         -  sync from host time %.3f sec", __func__, core->sync_from_device_ultra);


  //  fprintf(stderr, "\n[%s]        statistics of event length", __func__);
   /* for(int i=0; i < 60; i++)
        fprintf(stderr, "\n[%s]         -  [%d, %d): %d ", __func__, i*10000, (i+1)*10000,  core->statis_event_length[i]);
*/
