...
int32_t align(AlignedPair* out_2, char* sequence, int32_t sequence_len,
    ...
    for (size_t band_idx = 2; band_idx < n_bands; ++band_idx) {
        ...
        //if(sequence_len==1414&&band_idx<=5)
        // fprintf(stderr,"------------------band_idx  %d \n",band_idx);
    ...
    //   if(sequence_len==1414&&band_idx<=5)
    //		fprintf(stderr,"\n lower_left %d %d\n",band_lower_left[band_idx].event_idx,band_lower_left[band_idx].kmer_idx); 
    ...
    /*        if(sequence_len==1414&&band_idx<=5)
       { 
         ...
         fprintf(stderr,"scaling.scale %f level_mean %f shift %f\n",scaling.scale ,models[kmer_rank].level_mean,scaling.shift);  
         fprintf(stderr,"scaledLevel %f gp_mean %f gp_stdv %f\n",scaledLevel, gp_mean, gp_stdv ); 
         fprintf(stderr,"diag %f lp_step %f\n",diag,lp_step);        
	fprintf(stderr,"score %f %f %f model.mean %f lp_emission %f\n",score_d, score_u,score_l,models[kmer_rank].level_mean,lp_emission);
           }*/
	...
	//   if(BAND_ARRAY(band_idx,offset)<-pow(10.0,7))
	//	fprintf(stderr,"band score %f %d\n",BAND_ARRAY(band_idx,offset), n_bands);
     //   if(sequence_len==1414&&band_idx<=5)   
   //	 fprintf(stderr,"band_idx %d %d %f\n",band_idx,offset,max_score);
            fills += 1;
        }
    }
